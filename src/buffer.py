...
class Buffer:
    ...
    def insert(self, a):
        ...
        """
        if isinstance(a, list): # Check if a is of type list
            for e in a: # Append every single thing from the list
                self._insert_element(e)
        else:
            self._insert_element(a)

    def _insert_element(self, elem):
        if self._count == self._capacity:   # If at max capacity
            self._data = np.concatenate((self._data, self._data), axis=0)
            self._capacity = self._data.shape[0]    

        self._data[self._count] = elem  # add the element
        self._count += 1    # Increase counter
    ...
    def get(self) -> np.ndarray:
        ...
        """
        return self._data[: self._count]    # Get everything from the buffer

    def clear(self):
        """
        Clears the buffer.
        """
        self._count = 0     # Remove count

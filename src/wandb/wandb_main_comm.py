...
def run_with_config(config=None):
    ...
    with wandb.init(config=config):
        ...
        mconfig['dgn']['num_heads'] = config.num_heads
        mconfig['dgn']['att_layers'] = config.num_attention_layers
        mconfig['training']['step_between_train'] = config.step_between_train
        mconfig['epsilon_greedy']['epsilon_decay'] = config.epsilon_decay # Default 0.996, if we update epsilone each episode, then epsilon_decay should be smaller
        mconfig['epsilon_greedy']['epsilon_update_freq'] = config.epsilon_update_freq # Recommended: once per episode
        mconfig['training']['sequence_length'] = config.sequence_length # Length of sequence taken from the replay buffer
        ...
        mconfig['commnet']['comm_rounds'] = config.comm_rounds



        if mconfig['only_eval']['eval']:
            assert Path(mconfig['only_eval']['model_path']).exists()
            loaded_dict = torch.load(mconfig['only_eval']['model_path'], map_location='cpu')
            ...
            for key in mconfig:
                if key == "device":
                    print(f"{key}:  {mconfig[key]}")
                    continue
                print(key)
                for subkey in mconfig[key]:
                    print(f"\t{subkey}: {mconfig[key][subkey]}")
            
        # Base classes of parameters
        cbase = mconfig['base']
        cnetmon = mconfig['netmon']
        device = mconfig['device']
        ctar_update = mconfig['target_update']
        ceval = mconfig['evaluation']
        ctraining = mconfig['training']
        cbuffer = mconfig['buffer']
        clog_buffer = mconfig['log_buffer']
        ceps = mconfig['epsilon_greedy']
        ...
        if cbase['model_type'] == "dgn":
            ...
            model = DQN(agent_obs_size, cdgn['hidden_dim'], env.action_space.n, activation_function)
        else:
        ...
        if mconfig['only_eval']['eval']:
            assert Path(mconfig['only_eval']['model_path']).exists()
            load_state_dict(
                torch.load(mconfig['only_eval']['model_path'], map_location=device),
                model,
                netmon,
            )
        ...
        try:
            ...
            for step in tqdm(range(1, ctraining['total_steps']+ 1), miniters= 100, dynamic_ncols = True, disable=disable_prog): # Disable disables progress bar
                ...
                wandb.log({
                    "step": step,
                    "reward": mean_reward,
                    "loss_aux": loss_aux.detach().mean().item(),
                    "q_values": q_values.detach().mean().item(),
                    "q_target": q_target.detach().mean().item(),
                    "loss": loss.item(),
            ...
            if writer is not None:
                try:
                    ...
                    torch.save(get_state_dict(model, netmon, mconfig),
...
if __name__ == "__main__":
    ...
    "parameters": {
        "model_type": {"values": ["comm_net"]},
        "agg_type": {"values": ["sum", "gcn"]},
        "att_regularization_coeff": {"values": [0.03]},
        "num_heads": {"values": [6]},
        "comm_rounds": {"values": [6,8,12]},
        "num_attention_layers": {"values": [6]},
        "step_between_train": {"values": [5,10]},
        "epsilon_decay": {"min": 0.95, "max": 0.996},
        "epsilon_update_freq": {"values": [70,90]},
        "sequence_length": {"values": [16]},
        "gamma": {"min": 0.9, "max": 0.99},
        "mini_batch_size": {"values": [16, 32]},
        "iterations": {"values": [8,12]}
        }
    }
    
    # Create the sweep first (run this on a machine with internet access)
    sweep_id = wandb.sweep(sweep_config, project="letadla_priority_sampling2_only_commnet")

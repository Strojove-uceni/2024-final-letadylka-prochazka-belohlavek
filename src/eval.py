...
def evaluate(
    ...
    for ep in tqdm(range(episodes), disable=disable_progressbar):
        ...
        if isinstance(env.get(), Routing):
            ...
            aux_stats["sum_planes_per_node"] = env.sum_planes_per_node
            aux_stats["sum_planes_per_edge"] = env.sum_planes_per_edge
...
def save_plane_location_graph(
    ...
    nx_edges = nx.draw_networkx_edges(
        ...
        pos=pos,
        width=4,
        edge_color=sum_planes_per_edge / (np.sum(sum_planes_per_edge) * edge_weight),
    ...
    nx_nodes = nx.draw_networkx_nodes(
        G,
        pos=pos,
        node_color=sum_planes_per_node / np.sum(sum_planes_per_node),
...
def create_routing_plots(
        ...
        for step in episode.steps:
            ...
            if last_step is not None:
                for a in range(step.done.shape[0]):
                    if not step.done[a]:
                        diff = np.abs(
                            step.info["plane_distances"][a]
                            - last_step.info["plane_distances"][a]
                        )
                        d["plane_distance_delta"][step.n].append(diff)

            d["occupied_edges"][step.n].append(step.info["occupied_edges"])
            d["planes_on_edges"][step.n].append(step.info["planes_on_edges"])
            d["total_plane_size"][step.n].append(step.info["total_plane_size"])
            d["plane_distance_mean"][step.n].append(
                np.mean(step.info["plane_distances"])
            )
            d["plane_distance_max"][step.n].append(
                np.max(step.info["plane_distances"])
            )
            for a in range(step.act.shape[0]):
                plane_size = step.info["plane_sizes"][a]
                action = step.act[a]
                d["action_to_plane_size"][action].append(plane_size)
    ...
    plot_attribute(
        "planes_on_edges", "planes on edges", output_dir / "planes_on_edges.png"
    )
    plot_attribute(
        "total_plane_size", "Total plane size", output_dir / "plane_size.png"
    )
    plot_attribute(
        "delays_arrived_mean",
        "Delays of arrived planes",
    ...
    plot_attribute(
        "plane_distance_mean", "Mean distance", output_dir / "distance_mean.png"
    )
    plot_attribute(
        "plane_distance_max", "Max distance", output_dir / "distance_max.png"
    )
    plot_attribute(
    ...
    for action in d["action_to_plane_size"]:
        plt.hist(d["action_to_plane_size"][action], bins=100)
        plt.xlabel(f"plane sizes for action {action}")
        plt.ylabel("Counts")
        plt.savefig(output_dir / f"plane_size_act_{action}.png", bbox_inches="tight")
        plt.clf()

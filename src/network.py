...
class Network:
    ...
    def __init__(self, adjacency_matrix, distance_matrix) -> None:
    ...
    def build_network(self, adjacency_matrix, distance_matrix):
        ...
        for i in range(self.n_nodes):
            new_waypoint = Node() # TODO: get x, y from Tomas
        ...
        for i in range(self.n_nodes):
            ...
            self.nodes[i].neighbors = [int(i) for i in non_zero_positions]  # Create a list for the neighbors of that node - indicies of columns in the adjacency matrix
            
            # Add neighbor i into all nodes from the i-th neighbors
            for j in self.nodes[i].neighbors:
                if i not in self.nodes[j].neighbors:
                    self.nodes[j].neighbors.append(i)   # Add i as the neighbor of j


                    # Sorted by the index - create an edge between them
                    edge_distance = distance_matrix[i,j]
                    if i < j:
                        new_edge = Edge(i, j, edge_distance)    # Edge from i to j
                    else:
                        new_edge = Edge(j, i, edge_distance)

                    self.edges.append(new_edge)     # Add edge into the list of all edges
                    self.nodes[j].edges.append(t_edge)      # Number of the edge
                    self.n_nodes[i].edges.append(t_edge)    # Same number of the original
    ...
    def update_shortest_paths(self):
        ...
        """
        self.shortest_paths = dict(nx.shortest_paths(self.G, weight = "weight"))   # Calculates the shortest paths -> dict
        ...
        #         self.shortest_paths_weights[start][end] = path_weight(self.G, self.shortest_paths[start][end], "weight") # Calculate the weight between each shortest path
...
# path = dict(nx.shortest_path(G, weight="weight"))

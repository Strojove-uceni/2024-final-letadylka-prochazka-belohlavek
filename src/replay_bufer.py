...
class ReplayBuffer(object):
    ...
    def get_batch(
        self, batch_size, device, sequence_length=0, beta=1.0
        ...
        num_valid_sequences = self.count - sequence_length

        priorities = np.array([self.priorities[(buffer_start + i) % self.count] for i in range(num_valid_sequences)])
        ...
        sequence_start_indices = self._random_generator.choice(num_valid_sequences, batch_size, replace=True, p=probabilities)

         # Calculate IS weights for each sequence start index
        is_weights = (1.0 / (len(probabilities) * probabilities[sequence_start_indices])) ** beta
    ...
    def update_sequence_priorities(self, sequence_start_indicies, td_errors, sequence_length):
        for i, start_idx in enumerate(sequence_start_indicies):
            priority = np.abs(td_errors.detach().cpu().numpy()[i]).max()
            for offset in range(sequence_length):
                idx = (start_idx + offset) % self.count

...
if visible_gpus:
    allocated_gpus = [int(x) for x in visible_gpus.split(',') if x.strip().isdigit()]
    num_allocated = len(allocated_gpus)

if num_allocated <=1:
    raise ValueError("Error happended. Oh no.")
else:
    device_netmon = torch.device("cuda:0")
    device_main = torch.device("cuda:1")
    print(f"Using NetMon on {device_netmon} and main model on {device_main}.")
...
netmon = NetMon(node_obs_size,  # 'in_features' in init
                ...
                ).to(device_netmon)    # Move to device


# Get observations from the environment
summary_node_obs = torch.tensor(env.get_node_observation(), dtype=torch.float32, device=device_netmon).unsqueeze(0)
summary_node_adj = torch.tensor(env.get_nodes_adjacency(), dtype=torch.float32, device=device_netmon).unsqueeze(0)
summary_node_agent = torch.tensor(env.get_node_agent_matrix(), dtype=torch.float32, device=device_netmon).unsqueeze(0)
...
env = NetMonWrapper(env, netmon, cnetmon['start_up_iters'], device_main) # Maybe add device here to explicitly move into device main
...
if cbase['model_type'] == "dgn":
    model = DGN(agent_obs_size, cdgn['hidden_dim'], env.action_space.n, cdgn['heads'], cdgn['att_layers'], activation_function, cdgn['kv_values']).to(device_main)
elif cbase['model_type'] == "comm_net":
    ccom_net = config['commnet']
    model = CommNet(agent_obs_size, cdgn['hidden_dim'], env.action_space.n, comm_rounds=ccom_net['comm_rounds'], activation_fn=activation_function).to(device_main)
elif cbase['model_type'] == "dqn":
    model = DQN(agent_obs_size, cdgn['hidden_dim'], env.action_space.n, activation_function).to(device_main)
else:
...
if config['only_eval']['eval']:
    assert Path(config['only_eval']['model_path']).exists()
    load_state_dict(
        torch.load(config['only_eval']['model_path'], map_location=device_main),
        model,
        netmon,
    )

model_tar = copy.deepcopy(model).to(device_main)     # Create a deep copy of the current model == DGN
...
if node_aux_size > 0 and ctraining['aux_loss_coeff'] > 0:
    ...
    aux_model = aux_model.to(device_main)
...
try:
    ...
    for step in tqdm(range(1, ctraining['total_steps']+ 1), miniters= 100, dynamic_ncols = True, disable=disable_prog): # Disable disables progress bar
        ...
        if model_has_state:
            ...
            done_mask = ~torch.tensor(done, dtype=torch.bool, device=device_main).view(1,-1,1)
        ...
        loss_q = torch.zeros(1, device=device_main)
        loss_aux = torch.zeros(1, device=device_main)
        loss_att = torch.zeros(1, device=device_main)

        model.train()   # Set the model into the training mode
        netmon.train()  # Set the model into the training mode

        for t, (sequence_start_indices, batch, is_weights) in enumerate(buff.get_batch(mini_batch_size, device=device_main, sequence_length=sequence_length, beta=beta)):
            ...
            if t == 0:
                ...
                netmon.state = batch.node_state.to(device_netmon)
            else:
                # Reset netmon state if env episode was done in this step
                netmon.state = last_netmon_state * (~last_batch_episode_done).view(-1,1,1).to(device_netmon)

            # Run netmon step
            network_obs = netmon(batch.node_obs.to(device_netmon), batch.node_adj.to(device_netmon), batch.node_agent_matrix.to(device_netmon))   # Forward pass

            if aux_model is not None:
                # Netmon aux loss
                aux_prediction = aux_model(netmon.state.to(device_main))    # Netmon.state is (1,131,256) -> prediction is of shape (131,131)
            ...
            batch.obs[:, :, -net_obs_dim:] = network_obs.to(device_main)
            ...
            with torch.no_grad():
                ...
                next_network_obs = netmon(batch.next_node_obs.to(device_netmon), batch.next_node_adj.to(device_netmon), batch.next_node_agent_matrix.to(device_netmon))
                next_net_obs_dim = next_network_obs.shape[-1]   #   model_checkpoint_steps
                batch.next_obs[:, :, -next_net_obs_dim:] = next_network_obs.to(device_main)
            ...
            q_values = torch.gather(
                    q_values, -1, batch.action.unsqueeze(-1)
                ).squeeze(-1)
            
            # Combined value loss for each sample in the batch
            td_error = q_values - q_target

            is_weights_tensor = torch.tensor(is_weights, dtype=torch.float32).unsqueeze(1)

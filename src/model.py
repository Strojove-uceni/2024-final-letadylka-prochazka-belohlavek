...
class AttModel(nn.Module):
    ...
    def __init__(self, in_features, k_features, v_features, out_features, num_heads, activation_fn, vkq_activation_fn):
        ...
        self.fc_v = nn.Linear(in_features, v_features * num_heads)  # Values
        self.fc_k = nn.Linear(in_features, k_features * num_heads)  # Keys
        self.fc_q = nn.Linear(in_features, k_features * num_heads)  # Queries
...
class SimpleAggregation(nn.Module):
    """
        Simple sum aggregation of states.
    """
...
class DGN(nn.Module):
    """
        Deep graph neural network with attention.
    """

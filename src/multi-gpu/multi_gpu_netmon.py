...
class GNetMon(nn.Module):
    ...
    def forward(self, x, mask, node_agent_matrix, state=None, max_degree=None, no_agent_mapping = False):
        
        # This function contains steps (1), (2) and (3)
        h, last_neighbor_h, updated_state = self.update_node_states(x, mask, state)
        ...
        if no_agent_mapping: 
            return h, updated_state
        
        return GNetMon.output_to_network_obs(h, node_agent_matrix), updated_state
    ...
    def update_node_states(self, x, mask, state=None):
        """
        ...
        if state == None:
            # Init
            state = torch.zeros((batch_size, n_nodes, self.state_size), device = x.device) 
        
        # Reshape the state before further processing
        state = self.state_reshape_in(state, batch_size, n_nodes)  
        ...
        if self.rnn_type in ["lstm", "lnlstm"]:
            h0, cx0 = self.rnn_obs(h, (state[0], state[1])) # rnn.obs processes the encoded features h along with the previous states
            h, cx = h0, cx0
        ...
        if self.aggregation_def_type == 2:
            H, C = state[0], state[1] # Init of additional aggregation types 
        ...
        if self.rnn_type in ["lstm", "lnlstm"]:
            if self.rnn_carryover:
                updated_state = torch.stack((h1, cx1))     # Concatenating tensors along a new dimension
            else:
                updated_state = torch.stack((h0, cx0, h1, cx1))

        elif self.rnn_type == "gru":
            if self.rnn_carryover:
                updated_state = h1.unsqueeze(0)
            else:
                updated_state = torch.stack((h0.unsqueeze(0), h1.unsqueeze(0)))

        elif self.rnn_type == "gconvlstm":
            updated_state = torch.stack((H, C))

        elif self.rnn_type == "none":
            # store last node state for debugging and aux loss
            updated_state = h.unsqueeze(0)

        updated_state = self.state_reshape_out(updated_state, batch_size, n_nodes)

        return h, last_neighbor_h, updated_state

    def state_reshape_in(self, state, batch_size, n_agents):
        """
        ...
        return state.reshape(batch_size * n_agents, self.num_states, -1).transpose(0,1)

    def state_reshape_out(self, state, batch_size, n_agents):
        """
        ...
        return state.transpose(0, 1).reshape(batch_size, n_agents, -1)

...
from multi_gpu_netmon import DGN, MLP, CommNet, GNetMon, DQN
from routing import Routing
from wrapper import GNetMonWrapper
...
netmon = GNetMon(node_obs_size,  # 'in_features' in init
...
env = GNetMonWrapper(env, netmon, cnetmon['start_up_iters'])
...
if device == "cuda":
    ...
    print(f"Using {torch.cuda.device_count()} GPUs for aux model DataParallel.")
    aux_model = torch.nn.DataParallel(aux_model)
else: 
    raise ValueError("CUDA NOT SELECTED.")
...
try:
    ...
    for step in tqdm(range(1, ctraining['total_steps']+ 1), miniters= 100, dynamic_ncols = True, disable=disable_prog): # Disable disables progress bar
        ...
        if step % log_buffer_size == 0:
            ...
            if mean_reward > best_mean_reward:
                torch.save(
                    get_state_dict(model.module, netmon.module, config),
        ...
        for t, (sequence_start_indices, batch, is_weights) in enumerate(buff.get_batch(mini_batch_size, device=device, sequence_length=sequence_length, beta=beta)):
            ...
            if t == 0:
                ...
                external_netmon_state = batch.node_state
            else:
                # Reset netmon state if env episode was done in this step
                external_netmon_state = last_netmon_state * (~last_batch_episode_done).view(-1,1,1)

            # Run netmon step
            network_obs, updated_netmon_state = netmon(batch.node_obs, batch.node_adj, batch.node_agent_matrix, external_netmon_state)   # Forward pass
            ...
            last_netmon_state = updated_netmon_state
            ...
            with torch.no_grad():
                ...
                next_network_obs, _ = netmon(batch.next_node_obs, batch.next_node_adj, batch.next_node_agent_matrix)
            ...
            if hasattr(model, 'att_weights') and att_regularization_coeff > 0:
                ...
                if isinstance(model, torch.nn.DataParallel):
                    attention = F.log_softmax(torch.stack(model.module.att_weights), dim=-1)
                    target_attention = F.softmax(torch.stack(model_tar.module.att_weights), dim=-1)
                else:
        ...
        if hasattr(model.module if isinstance(model, torch.nn.DataParallel) else model, "att_weights") and ctraining['att_regularization_coeff'] > 0:
            log_info["loss_q"].insert(loss_q.item())
            log_info["loss_att"].insert(loss_att.item())

        # Update target model
        if ctar_update['steps'] <= 0:
            # Smooth target update
            interpolate_model(
                model.module if isinstance(model, torch.nn.DataParallel) else model,
                model_tar.module if isinstance(model_tar, torch.nn.DataParallel) else model_tar,
                ctar_update['tau'],
                model_tar)
            ...
            if isinstance(model, torch.nn.DataParallel):
                print("WITHIN")
                model_tar.module.load_state_dict(model.module.state_dict())
            else:
    ...
    if writer is not None:
        try:
            ...
            torch.save(get_state_dict(model.module, netmon.module, config),
